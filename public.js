...
async function submitFeedback(ev){
  ...
  payload.feedbackId = payload.feedbackId || createFeedbackId();
  ...
  try{
    ...
    try {
      ...
      // Apps Script sometimes saves successfully but the iframe response is not received by GitHub Pages.
      // Because the frontend generated the feedbackId in advance, we can verify whether the row exists.
      const verified = await verifySavedFeedback(payload.feedbackId);
      if(!verified.exists) throw postError;
      saved = { feedbackId: payload.feedbackId, verifiedAfterTimeout: true };
    }

    try {
      await uploadFiles(saved.feedbackId, payload);
    } catch(fileError) {
      RF.showAlert('#alert', 'ההתייחסות נשמרה, אך העלאת הקבצים נכשלה: ' + fileError.message, 'error');
      RF.qs('#saveState').textContent='ההתייחסות נשמרה, הקבצים לא עלו';
...
async function verifySavedFeedback(feedbackId){
  try { return await RF.jsonp('feedbackStatus', { feedbackId }); }
  catch(e) { return { exists:false, error:e.message }; }
}

function showSuccess(feedbackId){
  RF.qs('#saveState').textContent='נשלח בהצלחה';
  RF.qs('#feedbackForm').innerHTML=`<div class="thanks"><span class="thanks-icon">✅</span><h2>ההתייחסות התקבלה!</h2><p>תודה על מילוי הטופס. ההתייחסות שלך תועבר לצוות הפרויקט לבדיקה.</p><p>מספר פנייה:<br><span class="feedback-id">${RF.esc(feedbackId)}</span></p><p>צוות הפרויקט יסווג ויטפל בהתייחסות בהתאם להשפעה תכנונית, ביצועית, בטיחותית או בקשה להבהרה.</p></div>`;
}
